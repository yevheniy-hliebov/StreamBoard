...
public class DeckCanvasViewModel : ObservableObject, IDisposable
{
    ...
    public IRelayCommand<DeckButtonSlot> ClickButtonCommand { get; }
    public IRelayCommand<object?> CopyCommand { get; }
    public IRelayCommand<object?> PasteCommand { get; }
    public IRelayCommand<object?> CutCommand { get; }
        ...
        DeckEditorState editorState)
    {
        ...
        ClickButtonCommand = new RelayCommand<DeckButtonSlot>(async slot => await ExecuteClickButton(slot));

        CopyCommand = new RelayCommand<object?>(
            _ => ExecuteCopy(),
            _ => _editorState.IsEditorMode);

        PasteCommand = new RelayCommand<object?>(
            async _ => await ExecutePaste(),
            _ => _buttonService.CanPaste() && _editorState.IsEditorMode);

        CutCommand = new RelayCommand<object?>(
            async _ => await ExecuteCut(),
            _ => _editorState.IsEditorMode);

        DeleteCommand = new RelayCommand<object?>(
            async _ => await ExecuteDelete(),
            _ => _editorState.IsEditorMode);
    ...
    private async Task ExecuteClickButton(DeckButtonSlot? slot)
    {
        if (slot == null) return;
    ...
    private void OnEditorStatePropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        else if (e.PropertyName == nameof(DeckEditorState.IsEditorMode))
        {
