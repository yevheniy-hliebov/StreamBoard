...
{
    private readonly IntegrationsStorage _storage;
    private readonly IOBSConnectionService _obsService;

    public OBSSettingsViewModel(IntegrationsStorage storage, IOBSConnectionService obsService)
    {
        _storage = storage;
    ...
    private void OnObsStatusChanged(ConnectionStatus status)
    {
    ...
    public string ConnectionStateString
    {
        get
        {
            if (_obsService.Status == ConnectionStatus.NotConnected)
            {
                return "Not connected";
            }
            if (_obsService.Status == ConnectionStatus.Connecting)
            {
                return "Connecting...";
            }
            if (_obsService.Status == ConnectionStatus.Disconnecting)
            {
                return "Disconnecting...";
            }
            return _obsService.Status.ToString();
        }
    }

    public string ActionButtonText => _obsService.Status switch
    {
    ...
    public bool IsProcessing =>
        _obsService.Status == ConnectionStatus.Connecting || _obsService.Status == ConnectionStatus.Disconnecting;
    ...
    public ICommand ToggleConnectionCommand => new RelayCommand<object>(async _ =>
    {
        ...
        try
        {
            ...
            else await _obsService.ConnectAsync();
        }
        catch (Exception ex)
        {
    ...
    public bool CanEditSettings => _obsService.Status == ConnectionStatus.NotConnected || _obsService.Status == ConnectionStatus.Failed;
}