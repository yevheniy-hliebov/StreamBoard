...
public class VariableService : ObservableObject, IVariableService
{
    ...
    public void SetVariable(string name, VariableScope scope, string value, ActionExecutionContext? context = null)
    {
        ...
        if (scope == VariableScope.Runtime)
        {
            ...
            VariableChanged?.Invoke(this, new VariableChangedEventArgs(name, safeValue));
        }
        else if (scope == VariableScope.Temporary)
        {
            lock (_lockObject)
            {
                ...
                {
                    var newTemp = new Variable { Name = name, Scope = VariableScope.Temporary, Value = safeValue };
                    _temporaryCache[name] = newTemp;
                    System.Windows.Application.Current.Dispatcher.Invoke(() => TemporaryVariables.Add(newTemp));
                }
            }
            VariableChanged?.Invoke(this, new VariableChangedEventArgs(name, safeValue));
        }
        else
        {
            lock (_lockObject)
            {
                ...
                {
                    var newGlobal = new Variable { Name = name, Scope = VariableScope.Global, Value = safeValue };
                    _globalCache[name] = newGlobal;
                    System.Windows.Application.Current.Dispatcher.Invoke(() => GlobalVariables.Add(newGlobal));
                }
                SaveToStorage();
            }
            VariableChanged?.Invoke(this, new VariableChangedEventArgs(name, safeValue));
